...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs font-medium text-ink/85 capitalize whitespace-nowrap">
      <span className={'h-1.5 w-1.5 rounded-full shrink-0 ' + (dot[status] ?? 'bg-zinc-400')} />
      {label}
    </span>
  );
}
